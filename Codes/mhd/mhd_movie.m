...
% write_movie = 1 to write AVI movie file
...
ss3 = ss(3);
ss4 = ss(4);
f1=figure('Position',[ss3*.03 ss4/4 300 300]);
iframe=0;

% Loop over times
vxmax_plot=-1;
vymax_plot=-1;
i2dfld= 0;  % number of 2d field values plotted
for it= 1:nt
...
% (mod(a,b) gives the remaider of a/b.)
  if( nfldout>0)&( mod(it-1,nfldout)==0 ) 
    ...
    title(strcat('v_x and v_y at t =',num2str(t)));
    xlabel('x')
    ylabel('V_x')
...
% Make an avi movie w/ 4 frames per sec
if( write_movie==1 )
  movie2avi(mov,'mhd_mov','fps',4);
end
